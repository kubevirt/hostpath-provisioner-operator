...
// List lists all HostPathProvisioners in the indexer.
func (s *hostPathProvisionerLister) List(selector labels.Selector) (ret []*v1alpha1.HostPathProvisioner, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.HostPathProvisioner))
	})
	return ret, err
}

// Get retrieves the HostPathProvisioner from the index for a given name.
func (s *hostPathProvisionerLister) Get(name string) (*v1alpha1.HostPathProvisioner, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("hostpathprovisioner"), name)
	}
	return obj.(*v1alpha1.HostPathProvisioner), nil
}
